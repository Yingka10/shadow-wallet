...
export const PLANNING_COPY = {
   ...
   * 期限那一輪（P1-A1）。
   *
   * 存在的理由：「把哈利波特讀完」有終點，但孩子不會主動說「兩週」。
   * 沒有這一頁，那個目標會被建成日常任務。
   *
   * ⚠️ 兩個尾巴是**客戶端固定加的**，模型改不掉，也不可以被關掉 ——
   *    跟 needs_choice 的「我有自己的方式」同一個地位。
   */
  duration: {
    hint: '選一個最接近的就好。',
    microcopy: '之後想改也可以，先抓一個大概。',
    /** 與 needs_choice 同一個理由：選起來與送出去是兩個動作。 */
    confirm: '下一步：整理成我的計畫',
    /** 尾巴一：他心裡有一個數字。 */
    customTitle: '我想自己說',
    customSubtitle: '我心裡有一個時間',
    customQuestion: '你想花多久？',
    customPlaceholder: '例如：21',
    customUnit: '天',
    customNext: '就這樣',
    /**
     * 尾巴二：**這不是「算不出來」的預設值**，是他的判斷 ——
     * 「沒有終點」會變成一個持續的日常安排，不是一段有終點的挑戰。
     */
    openEndedTitle: '這件事沒有終點',
    openEndedSubtitle: '我想一直做下去',
    /**
     * 超出範圍時**擋下並說明**，不悄悄改成 180 ——
     * 那等於讓他確認一個他沒說過的期限。
     */
    outOfRange: '先填 1 到 180 天之間，好嗎？',
    notANumber: '填一個數字就好，例如 21。',
    reassurance: '🌱 之後也可以再調整，沒關係！',
  },
