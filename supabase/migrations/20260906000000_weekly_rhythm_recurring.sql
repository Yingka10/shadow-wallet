...
--
-- 三處要一起改，少一處都沒有效果：
--   1. child_proposal_plan_versions 的 evidence CHECK（20260812）
--   2. publish_child_confirmed_plan_v1 的推導（現行定義在 20260828:416）
--   3. propose_child_planning_terms_v1 的推導（現行定義在 20260830:414）
--
-- 下面兩支函式是從上述現行定義**逐字複製**過來的，只改了那一段條件。
-- CREATE OR REPLACE 是全體置換，貼片段會把函式其餘部分刪掉。
--
-- 冪等：DROP CONSTRAINT IF EXISTS 與 CREATE OR REPLACE 都可重跑。
-- ─────────────────────────────────────────────────────────────────────────

-- ── 1. evidence CHECK 的值域 ─────────────────────────────────────────────
...
-- ── 2. publish_child_confirmed_plan_v1 ───────────────────────────────────
...
COMMENT ON FUNCTION public.publish_child_confirmed_plan_v1(jsonb) IS
  ...
  '2026-09-06：weekly_rhythm 只排除 one_time，recurring 也能有每週節奏。';
...
-- ── 3. propose_child_planning_terms_v1 ───────────────────────────────────
