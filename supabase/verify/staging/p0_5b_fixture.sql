-- P0-5B staging acceptance — 隔離 fixture。
--
-- 為什麼要自己開一個家庭，而不是沿用既有的孩子：
-- P0-5B 的驗收本身是破壞性的（會建立 / 關閉 / 重放大量提案與正式任務），
...
--
-- 密碼由執行端以 __P0_5B_PASSWORD__ 佔位符替換，不進 git。
--
-- 跑法：
--   python -c "…替換佔位符…" > run.sql && supabase db query --linked -f run.sql
--   驗完務必跑 p0_5b_cleanup.sql。

DO $fixture$
DECLARE
  v_pw       text := '__P0_5B_PASSWORD__';
  v_email    text := 'p0-5b-verify@example.invalid';
  v_user     uuid;
  v_family   uuid;
  v_child    uuid;
BEGIN
  IF v_pw = '__P0_5B_' || 'PASSWORD__' OR length(btrim(v_pw)) < 6 THEN
    RAISE EXCEPTION '密碼佔位符沒有被替換，或密碼太短';
  END IF;

  IF EXISTS (SELECT 1 FROM auth.users WHERE email = v_email) THEN
    RAISE EXCEPTION 'P0-5B fixture 已存在，先跑 p0_5b_cleanup.sql';
  END IF;
  ...
  INSERT INTO families (family_name, created_by)
    VALUES ('P0-5B Verify Family', v_user) RETURNING id INTO v_family;

  INSERT INTO parents (family_id, user_id, name, email)
    VALUES (v_family, v_user, 'P0-5B Verify Parent', v_email);

  INSERT INTO children (family_id, nickname, birth_date, age_group)
    VALUES (v_family, 'P0-5B Kid',
  ...
  RAISE NOTICE 'P0-5B family = %  child = %  user = %', v_family, v_child, v_user;
END
$fixture$;
