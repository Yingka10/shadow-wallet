-- P0-5B staging acceptance — 清除隔離 fixture。
--
...
--
-- 這支只碰 'P0-5B Verify Family'。Demo Family 與 QA Family 完全不在範圍內。
...
DECLARE
  ...
  v_left   int;
BEGIN
  SELECT id INTO v_family FROM families WHERE family_name = 'P0-5B Verify Family';
  SELECT id INTO v_user   FROM auth.users WHERE email = v_email;

  IF v_family IS NULL AND v_user IS NULL THEN
    RAISE EXCEPTION 'P0-5B CLEANUP：沒有東西要清（fixture 不存在）';
  END IF;
  ...
  SELECT
      (SELECT count(*) FROM families   WHERE family_name = 'P0-5B Verify Family')
  ...
  IF v_left <> 0 THEN
    RAISE EXCEPTION 'P0-5B CLEANUP 失敗：還有 % 筆殘留或孤兒（整包已回滾）', v_left;
  END IF;

  RAISE NOTICE 'P0-5B CLEANUP 完成，殘留與孤兒皆為 0';
END
$cleanup$;
