...
4. **motivation_observation（headline）要寫成三段，各自負責不同的事，不能只是一句空泛結論：**
   - **第一段：總覽判斷**——這週大致穩不穩定、有沒有一條特別值得留意。**有 focus line 時，這段的任務是讓家長一眼看出「這條線今天值得花時間看」，不是講成一句順口帶過的觀察**。不要用「比較需要我們一起留意一下」「可以稍微關注」這種輕描淡寫、像是順便一提的講法——那種語氣會讓家長覺得「聽起來沒什麼事」，跟畫面上特別標成「值得一起看看」的用意矛盾。改寫成明確點名、语氣肯定的講法，例如「這週有一條線特別值得一起看看：OO」「這週多數線都穩，OO這條線這週比較需要花時間聊聊」。needs_discussion 比 watch 語氣更明確一階——needs_discussion 是「值得一起看看」，watch 才是語氣較輕的「先觀察」。
   ...
   - **第三段：focus line 的具體診斷 + 下一步**——這段是全文重點，**只能照 facts 字面講到哪裡，不能多推一步下診斷**，但也不能寫得像「其實還好、不用特別做什麼」：
     ...
     · **段落最後的下一步要寫成一個具體、家長讀完會想去做的動作**，不要只停在「先看看是不是合適」這種懸而未決的觀察句——那種收尾會讓整段讀起來像「其實沒事」。優先用「找時間跟孩子聊聊」「這週可以主動問問他」這種指向一次真實對話或動作的講法，接在事實後面，例如「這一條這週比較值得找時間跟孩子聊聊，看看提醒的時機或方式要不要調整」。**「其他安排先維持即可」這句可以留，但只能當這句的前導、不能是整句的唯一內容**——不要讓下一步聽起來只是「其他都不用管」而已。
...
function computeFallbackInsight(ctx: WeeklyContext): GeminiInsightResult {
  ...
  const nextStep = focusLine
    ? `其他安排先維持即可，找時間跟孩子聊聊「${focusLine.label}」，看看提醒的時機或方式要不要調整。`
    : '';
