...
6. 只有在上面的跨週資料至少有兩個可比較週期時，才能描述趨勢；只有一週就只陳述那一週。沒有列出的週或任務不等於沒有做，不能自行補成零次。
7. 「較早的跨週脈絡」只用來辨識變化，不能當成孩子現在仍維持相同狀態。若較新與較舊資料不同，以較新資料為準。
8. 不要把「曾經需要提醒」寫成孩子現在一定仍需要提醒，也不要從歷史紀錄推論孩子的動機、性格或家庭關係。
9. 「最近一次已確認的共同版本變動」只能當成家庭曾經做過的決定，不代表它造成後續表現；除非紀錄本身足夠，不能自行寫成因果關係。
